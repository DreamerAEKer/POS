...
const App = {
    ...
    renderSettingsView: (container) => {
        ...
        container.innerHTML = `
            <div style="padding:20px;">
                <h2>ตั้งค่าร้านค้า (Settings)</h2>
                
                <div class="form-group">
                    <label>ชื่อร้าน</label>
                    <input type="text" id="settings-store-name" value="${settings.storeName}" style="width:100%; padding:8px;">
                </div>

                <div class="form-group">
                    <label>อัตราภาษี (%)</label>
                    <input type="number" id="settings-tax" value="${settings.taxRate}" style="width:100%; padding:8px;">
                </div>

                <!-- Printer Settings Section -->
                <div style="margin-top:20px; border-top:1px solid #eee; padding-top:10px;">
                    <h4 style="margin-bottom:10px;">ตั้งค่าเครื่องพิมพ์ (Printer)</h4>
                    
                    <div class="form-group">
                        <label>เว้นบรรทัดท้ายใบเสร็จ (Feed Lines)</label>
                        <div style="display:flex; align-items:center; gap:10px;">
                            <input type="range" id="settings-feed-range" min="0" max="20" value="${settings.printerFeedLines || 5}" 
                                oninput="document.getElementById('settings-feed-val').textContent = this.value" style="flex:1;">
                            <span id="settings-feed-val" style="width:30px; text-align:center;">${settings.printerFeedLines || 5}</span>
                        </div>
                        <div style="font-size:12px; color:#666; margin-top:5px;">
                            ปรับเพิ่มถ้าเครื่องตัดกระดาษโดนข้อความ (มาตรฐาน 5-8 บรรทัด)
                        </div>
                    </div>
                </div>

                <div class="form-group" style="margin-top:20px;">
                    <label>รูปโลโก้หัวบิล (Logo)</label>
                    
                    ${settings.logo ? `
                        <div style="margin-bottom:10px; position:relative; width:100px;">
                            <img src="${settings.logo}" style="width:100px; border:1px solid #ddd;">
                            <button class="icon-btn dangerous" style="position:absolute; top:-10px; right:-10px; width:24px; height:24px;" 
                                onclick="App.confirm('ลบรูปโลโก้?').then(yes => { if(yes) { DB.saveSettings({...DB.getSettings(), logo: null}); App.renderSettingsView(document.getElementById('pos-view-container')); } })">
                    ...
                    <div style="display:flex; align-items:center; gap:10px;">
                        <input type="file" id="settings-logo-upload" accept="image/*" style="display:none;" onchange="App.handleImageUpload(this, 's-logo-preview')">
                        <button class="secondary-btn" onclick="document.getElementById('settings-logo-upload').click()">
                            <span class="material-symbols-rounded">upload</span> อัพโหลดรูปใหม่
                        </button>
                    </div>
                    <img id="s-logo-preview" style="max-width:100px; margin-top:10px; display:none;">
                </div>

                <!-- Group Images Management -->
                <div style="margin-top:20px; border-top:1px solid #eee; padding-top:15px;">
                    <h4 style="margin-bottom:15px;">รูปภาพหมวดหมู่ (Category Images)</h4>
                    <div id="settings-group-list" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap:15px;">
                        <!-- Generated by JS -->
                    </div>
                </div>

                <button class="primary-btn" style="width:100%; margin-top:30px;" id="btn-save-settings">บันทึกตั้งค่า</button>
            </div>
        `;

        // Load Group Images UI
        const groups = [...new Set(DB.getProducts().map(p => p.group))];
        const groupImages = DB.getGroupImages();
        const groupList = document.getElementById('settings-group-list');

        groups.forEach(group => {
            const hasImage = groupImages[group];
            const div = document.createElement('div');
            div.style.cssText = 'border:1px solid #ddd; padding:10px; border-radius:8px; text-align:center; position:relative;';
            div.innerHTML = `
                <div style="font-weight:bold; margin-bottom:5px; font-size:14px;">${group}</div>
                <div style="height:80px; background:#f9f9f9; display:flex; align-items:center; justify-content:center; margin-bottom:5px; overflow:hidden; border-radius:4px;">
                    ${hasImage ? `<img src="${hasImage}" style="width:100%; height:100%; object-fit:cover;">` : '<span style="color:#ccc;">ไม่มีรูป</span>'}
                </div>
                <button class="secondary-btn small" onclick="App.triggerGroupUpload('${group}')" style="width:100%;">
                    ${hasImage ? 'เปลี่ยนรูป' : 'เพิ่มรูป'}
                </button>
                ${hasImage ? `
                    <button class="icon-btn dangerous" style="position:absolute; top:-5px; right:-5px; width:20px; height:20px;" 
                        onclick="App.removeGroupImage('${group}')">
        ...
        document.getElementById('btn-save-settings').onclick = async () => {
            ...
            const taxRate = parseFloat(document.getElementById('settings-tax').value);
            const feedLines = parseInt(document.getElementById('settings-feed-range').value) || 5;

            // Handle Logo Upload
            const logoPreview = document.getElementById('s-logo-preview');
            let newLogo = settings.logo;
            if (logoPreview.src && logoPreview.style.display !== 'none') {
                newLogo = logoPreview.dataset.base64; // Assuming standard preview logic
                // If standard logic isn't used, we might need Utils.fileToBase64 manually here.
                // But let's assume handleImageUpload does the job of setting src and dataset.base64
            }
            ...
            DB.saveSettings({
                ...
                storeName,
                taxRate,
                printerFeedLines: feedLines,
                logo: newLogo
            });

            await App.alert('บันทึกการตั้งค่าเรียบร้อย!');
            App.renderPOSView();
        };
    },

    triggerGroupUpload: (groupName) => {
        ...
        input.onchange = async (e) => {
            if (e.target.files[0]) {
                ...
                App.renderSettingsView(document.getElementById('pos-view-container'));
            }
        };
        input.click();
    },
    ...
    removeGroupImage: (groupName) => {
        App.confirm(`ลบรูปหมวดหมู่ "${groupName}"?`).then(yes => {
            if (yes) {
                DB.removeGroupImage(groupName);
                App.renderSettingsView(document.getElementById('pos-view-container'));
            }
