...
const App = {
    ...
    // --- Camera Scanner Logic (Integrated from scanner.html) ---
    scannerState: {
        codeReader: null,
        scanning: false,
    ...
    openCameraScanner: async () => {
        ...
        App.scannerState.scanning = true;
        App.setScannerStatus('กำลังเริ่มกล้อง...', '');
        
        // Hide mobile bottom nav
        const botNav = document.getElementById('mobile-bottom-nav');
        if (botNav) botNav.style.display = 'none';

        // Bind events if not already done
        if (!App.scannerState.eventsBound) {
            document.getElementById('btn-scanner-close').addEventListener('click', App.closeCameraScanner);
            document.getElementById('btn-scanner-cam-sw').addEventListener('click', App.switchScannerCam);
            document.getElementById('btn-scanner-torch').addEventListener('click', App.toggleScannerTorch);
            App.scannerState.eventsBound = true;
        }

        try {
            await App.startScannerDecode();
        } catch (err) {
            console.error('Camera open error:', err);
            document.getElementById('scanner-video').style.display = 'none';
            document.getElementById('scanner-no-cam').style.display = 'flex';
            App.setScannerStatus('ไม่สามารถเปิดกล้องได้', 'err');
        }
    },

    closeCameraScanner: () => {
        App.scannerState.scanning = false;
        App.scannerState.nativeDetecting = false;
        
        if (App.scannerState.codeReader) {
            try { App.scannerState.codeReader.reset(); } catch (e) {}
        }
        ...
        // Restore bottom nav display based on screen width
        const botNav = document.getElementById('mobile-bottom-nav');
        if (botNav && window.innerWidth <= 1024) {
            botNav.style.display = 'flex';
        }
        
        // Restore torch button styling
        App.scannerState.torchOn = false;
        const torchBtn = document.getElementById('btn-scanner-torch');
        if (torchBtn) torchBtn.classList.remove('active');
        
        // Refocus global search
        if (App.elements.globalSearch) {
            App.elements.globalSearch.focus();
        }
    },

    setScannerStatus: (msg, cls) => {
        const hud = document.getElementById('scanner-status-hud');
        if (hud) {
            hud.textContent = msg;
            hud.className = 'scanner-hud-status ' + (cls || '');
            if (cls === 'ok') {
                hud.style.borderColor = '#00e676';
                hud.style.color = '#00e676';
            } else if (cls === 'err') {
                hud.style.borderColor = '#ff5252';
                hud.style.color = '#ff5252';
            } else {
                hud.style.borderColor = 'rgba(255, 255, 255, 0.15)';
                hud.style.color = 'rgba(255, 255, 255, 0.85)';
            }
        }
    },

    startScannerDecode: async () => {
        if (App.scannerState.codeReader) {
            try { App.scannerState.codeReader.reset(); } catch (e) {}
        }
        ...
        const constraints = {
            video: {
                facingMode: App.scannerState.facingEnvironment ? 'environment' : 'user',
                width: { ideal: 1280 },
                height: { ideal: 720 },
        ...
        if ('BarcodeDetector' in window) {
            try {
                if (!App.scannerState.nativeBarcodeDetector) {
                    App.scannerState.nativeBarcodeDetector = new BarcodeDetector({
                ...
                document.getElementById('scanner-no-cam').style.display = 'none';
                App.setScannerStatus('พร้อมสแกน (Native) ✓', 'ok');
                
                App.scannerState.nativeDetecting = true;
                App.detectScannerNativeBarcode();
                return;
            } catch (err) {
        ...
        if (typeof ZXing === 'undefined') {
            App.setScannerStatus('เครื่องสแกนกล้องยังไม่พร้อมใช้งาน (โหลดไฟล์ไม่สำเร็จ)', 'err');
            return;
        }
        
        if (!App.scannerState.codeReader) {
            App.scannerState.codeReader = new ZXing.BrowserMultiFormatReader();
        }
        
        try {
            await App.scannerState.codeReader.decodeFromConstraints(constraints, 'scanner-video', (res) => {
                if (res && App.scannerState.scanning) {
                    const r = res.getText();
                    if (r && r !== App.scannerState.lockBarcode) {
                        App.handleScannerBarcode(r);
                    }
                }
            });
            video.style.display = 'block';
            document.getElementById('scanner-no-cam').style.display = 'none';
            App.setScannerStatus('พร้อมสแกน (ZXing) ✓', 'ok');
        } catch (err) {
            try {
                await App.scannerState.codeReader.decodeFromVideoDevice(undefined, 'scanner-video', (res) => {
                    if (res && App.scannerState.scanning) {
                        const r = res.getText();
                        if (r && r !== App.scannerState.lockBarcode) {
                            App.handleScannerBarcode(r);
                ...
                video.style.display = 'block';
                document.getElementById('scanner-no-cam').style.display = 'none';
                App.setScannerStatus('พร้อมสแกน (ZXing สำรอง) ✓', 'ok');
            } catch (fallbackErr) {
                video.style.display = 'none';
                document.getElementById('scanner-no-cam').style.display = 'flex';
                App.setScannerStatus('เปิดกล้องไม่ได้: ' + (err.message || err), 'err');
            }
        }
    },

    detectScannerNativeBarcode: async () => {
        if (!App.scannerState.nativeDetecting || !App.scannerState.nativeBarcodeDetector) return;
        
        const video = document.getElementById('scanner-video');
        if (video && video.readyState === video.HAVE_ENOUGH_DATA && App.scannerState.scanning) {
            try {
                const barcodes = await App.scannerState.nativeBarcodeDetector.detect(video);
                if (barcodes.length > 0) {
                    const r = barcodes[0].rawValue;
                    if (r && r !== App.scannerState.lockBarcode) {
                        App.handleScannerBarcode(r);
                    }
                }
        ...
        if (App.scannerState.nativeDetecting) {
            requestAnimationFrame(App.detectScannerNativeBarcode);
        }
    },

    switchScannerCam: async () => {
        App.scannerState.facingEnvironment = !App.scannerState.facingEnvironment;
        App.setScannerStatus('กำลังสลับกล้อง...', '');
        await App.startScannerDecode();
    },

    toggleScannerTorch: async () => {
        const video = document.getElementById('scanner-video');
        const stream = video ? video.srcObject : null;
        if (!stream) {
            await App.alert('กล้องยังไม่พร้อม');
            return;
        }
        ...
        if (!caps.torch) {
            await App.alert('กล้องนี้ไม่มีไฟฉาย');
            return;
        }
        App.scannerState.torchOn = !App.scannerState.torchOn;
        try {
            await track.applyConstraints({ advanced: [{ torch: App.scannerState.torchOn }] });
            document.getElementById('btn-scanner-torch').classList.toggle('active', App.scannerState.torchOn);
        } catch (e) {
            await App.alert('เปิดไฟฉายไม่ได้');
            App.scannerState.torchOn = false;
        }
    },

    handleScannerBarcode: (barcode) => {
        if (!App.scannerState.scanning) return;
        
        App.scannerState.lockBarcode = barcode;
        clearTimeout(App.scannerState.lockClearTimer);
        App.scannerState.lockClearTimer = setTimeout(() => {
            App.scannerState.lockBarcode = null;
        }, 3000);
        
        App.setScannerStatus('อ่านได้: ' + barcode, 'ok');
        
        if (navigator.vibrate) navigator.vibrate(40);
        
        // Successfully scanned, close scanner and process
        App.closeCameraScanner();
        App.handleBarcodeScan(barcode);
    },
    ...
    setupGlobalInput: () => {
        ...
        // 3. Manual Trigger Button - Wire to start camera scanner overlay
        document.getElementById('btn-scan-trigger').addEventListener('click', async () => {
            App.openCameraScanner();
        });
    },
